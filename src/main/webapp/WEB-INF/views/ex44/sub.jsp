...
<body>
	...
	<input type="text" name="name" id="nameInput2" value="박지성"> <br>
	<input type="text" name="location" id="locationInput2" value="런던"> <br>
	<input type="text" name="since" id="sinceInput2" value="2000년"> <br>
	</form>
	...
	<input type="datetime-local" id="dateTime14"> <br>
...
document.querySelector("#btn13").addEventListener("click", function() {
	...
	const hasCarCheck = document.querySelector("#hasCarCheckBox3:checked");
	const hasCar = hasCarCheck != null;
	const foodCheck = document.querySelectorAll(".foodCheckbox3:checked");
	const food = [];
	
	
	
	for (let i = 0; i < foodCheck.length; i++){
		food.push(foodCheck[i].value);
	}
	
	const obj = {age : age, name : name, hasCar : hasCar, food : food};
...
document.querySelector("#btn12").addEventListener("click", function() {
	...
	const obj = {name : name, job : {location : location, since : since}};
	const data = JSON.stringify(obj);
...
document.querySelector("#btn10").addEventListener("click", function() {
	const o = {name : "park", hasCar : true, food : ["김치", "탕수육"], age : 40};
